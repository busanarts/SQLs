...
MAX(NVL(DECODE(grp, '당초입학', CNT, 0),0)) +
...
MAX(NVL(DECODE(grp||학적변동, '총인원53', CNT, 0),0)) 재학생총인원,
...
-- 당초입학
  ...
  UNION
  -- 재학생학적변동
	SELECT '총인원' grp, HJ_HAKNYUN 학년, 학과명, HC_SCODE 학적변동, COUNT(*) CNT
  FROM AC12011_20141001, AC1121, TBL학과, (

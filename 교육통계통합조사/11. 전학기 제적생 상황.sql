SELECT 학과명, 성별, 학번, 학년, f정원내외(학번) 정원내외, f전형구분(학번) 전형구분, f전형상세(학번) 전형상세, 성 || 이름 성명, f_statenm(학적상태) 학적상태,
usr_todate(SUBSTR(주민등록번호,1,6)) 생년월일, age(substr(주민등록번호,1,6), SUBSTR(:조사기준일,1,4) || '0301') 연령,
	...
	(
		...
		WHERE changedt >= substr(:조사기준일,1,4) -1 || '0401' AND changedt <= substr(:조사기준일,1,4) -1 || '0930'

...
ADMISSIONFEE + TUITION 원등록금액, ADMISSIONFEE 입학금, TUITION 수업료, SCHOLARSHIP 고지장학금,
F_janghaknm(janghakcd1) 장학명1, DECODE(TRIM(GBN1), '1', '선감면', '2', '후지급') 장학구분1, SCHOLARSHIP1 장학1금액,
F_janghaknm(janghakcd2) 장학명2, DECODE(TRIM(GBN2), '1', '선감면', '2', '후지급') 장학구분2, SCHOLARSHIP2 장학2금액,
F_janghaknm(janghakcd3) 장학명3, DECODE(TRIM(GBN3), '1', '선감면', '2', '후지급') 장학구분3, SCHOLARSHIP3 장학3금액,
F_janghaknm(janghakcd4) 장학명4, DECODE(TRIM(GBN4), '1', '선감면', '2', '후지급') 장학구분4, SCHOLARSHIP4 장학4금액,
(NVL(SCHOLARSHIP1,0) + NVL(SCHOLARSHIP2,0) + NVL(SCHOLARSHIP3,0) + NVL(SCHOLARSHIP4,0)) 장학금계,
ADMISSIONFEE + TUITION - SCHOLARSHIP - NVL(PREFEE,0) 고지금액, NVL(수납금액,0) "수납금액(낸돈)",
DECODE(NVL(수납금액,0), 0, 0, NVL(수납금액,0) - NVL(환불금액,0)) "실납부금액(낼돈)", NVL(환불금액,0) 환불금액,
DECODE(고지명부.state, '77', '완납', '11', '미납', '33', '납부유예', '22', '부분납', '55', '승계', '88', '환불') 납부구분, 수납일자, 환불일자,
DECODE(고지명부.state, '11', 0, -(ADMISSIONFEE + TUITION - SCHOLARSHIP - NVL(수납금액,0) + NVL(환불금액,0))) 금액오류체크, SCHOLARSHIP -
(DECODE(TRIM(GBN1), '1', SCHOLARSHIP1, 0) + DECODE(TRIM(GBN2), '1', SCHOLARSHIP2, 0) + DECODE(TRIM(GBN3), '1', SCHOLARSHIP3, 0) +DECODE(TRIM(GBN4), '1', SCHOLARSHIP4, 0)) 장학금오류체크,
